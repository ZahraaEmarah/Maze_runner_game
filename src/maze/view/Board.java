	...
	public class Al implements KeyListener
	{
		...
		public void keyPressed(KeyEvent e) {
			...
			if(keycode == KeyEvent.VK_P)
			{
				...
				JOptionPane.showOptionDialog(m.f, "", "PAUSE", JOptionPane.CLOSED_OPTION, JOptionPane.PLAIN_MESSAGE, 
						new ImageIcon("./src/pics/donaldWait.gif"), options, options[0] );
			  ...
			  if(Scarybomb1 && !hitscarybomb1)
			   ...
			   // 	m.h.setHealth("scary");
				...
				JOptionPane.showOptionDialog(m.f, "", "BOMBED", JOptionPane.CLOSED_OPTION, JOptionPane.PLAIN_MESSAGE, 
						new ImageIcon("./src/pics/seriously.gif"), options, options[0] );
				}
			}
			...
			if(m.h.gethealth() <=50)
			{
				m.trump.setIcon(new ImageIcon("./src/pics/HalfHalf.gif"));
			}
			...
			if(p.gettileX() == 28 && p.gettileY() == 19)
			{
				...
				m.m1.stop();
	              ...
	              new ImageIcon("./src/pics/YouWin.gif"), options, options[0] );
    	...
    	//	c.listOfplayers.remove(m.ip);
                ...
                if(m.playerexist==false) {  pw.frame.setVisible(true);}
				  ...
				  m.trump.setIcon(new ImageIcon("./src/pics/Happy.gif"));
                  bo.initialize();
                  m.sec = 1;
                  m.s1.restart(); 
                  m.min =0;
			 ...
			 if (m.h.gethealth()<=0)
			{
			      ...
			      new ImageIcon("./src/pics/loser.gif"), options, options[0] );
				  System.exit(0);
			}
